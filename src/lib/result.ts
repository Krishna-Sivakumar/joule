...
export function Bind<OriginalValue, TransformedValue>(
	maybe: Result<OriginalValue>,
	transform: (val: OriginalValue) => Result<TransformedValue>
): Result<TransformedValue> {
	if (isOk(maybe)) {
...
export function UnwrapDefault<T>(maybe: Result<T>, _default: T) {
	return Match(maybe, {
		onOk: (val) => val,
		onErr: (_) => _default
	})
...
export const MapError = <V>(r: Result<V>, func: (err: Error) => Error) =>
	Match(r, {
		onOk(_) {
			return r;
		},
